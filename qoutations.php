...
<div class="edit_profile_container" id="test"> 
		...
		<label class="pickers">
				  <input type="checkbox" class="kuronga" value="Price">Price
				</label>
		<label class="pickers">
				  <input type="checkbox" class="kuronga" value="Lead">Lead time
				</label>
		<label class="pickers">
				  <input type="checkbox" class="kuronga" value="Rating">Ratings
				</label>
...
<form id="" method="post"  enctype="multipart/form-data" >
	<fieldset >
      <button style="float: right;" name="submit" type="submit" id="contact-submit" data-submit="...Sending">Short list</button>
    </fieldset>	
...
<?php
	if (isset($_GET['requestid'])){
			...
			$fproduct="select * from qoutations where cusid = '$taskOwner' and requestid='$reqid'";
	...
	?>
       <tr>
       		<td><a href="qoutations.php?requestid=<?php echo $row['requestid'];?>"><?php echo $row['requestid'].'-'.$row['pname'];
		  ...
		  <td><?php echo $row['subtotal']; ?></td>
		   ...
		   <td><?php echo $row['total']; ?></td>
       		<td><a href="<?php echo $row['qoute'];?>">Download</a></td>
   <td><input type="checkbox" name="check_list[]" value="<?php echo $row['supid'];?>|<?php echo $row['requestid'];?>"></td>
	...
	$qry = "select * from qoutations where cusid = '$taskOwner' and status = 'Sent'";
	...
	?>
       <tr>
		  ...
		  <td><?php echo $row['subtotal']; ?></td>
		   <td><?php echo $row['total']; ?></td>
       		<td><a href="<?php echo $row['qoute'];?>">Download</a></td>
   <td><input type="hidden" name="check_list[]" value="<?php echo $row['supid'];?>"></td>
...
?>
   ...
   <label>
    <h4 style="font-size: 18px">Shortlist</h4></label>
   ...
   <ul>
	...
	$checked = $_POST['check_list'];
	   ...
	   <li><?php echo $result_explode[0];?></li> 
	   <?php
...
// Code added for adding class to sibling elements as "group_<id>"  
	 ...
	 $(".kuronga").on("change", function () {
    //gets the value of checked box
        var kuronga = $(".kuronga:checked").map(function () {
            return $(this).val()
        }).get();
        
		  var all = table.order([1, "asc"]).draw();
